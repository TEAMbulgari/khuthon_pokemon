...
def upload(request):
    if request.method == 'POST' and request.FILES['photo']:
        ...
        r, p, i = getResult()
        return render(request, 'show.html',{'result':r, 'prob':p*100, 'imagefile':i, 'MEDIA_URL':settings.MEDIA_URL})
...
def show(request):
    r, p, i = getResult()
    return render(request, 'show.html',{'result':r, 'prob':p*100, 'imagefile':i, 'STATIC_URL':settings.STATIC_URL})
...
def getResult():
    ...
    index=jpg_index[0]
    print('done')
    return name,probability,'ori_'+name+'_'+str(index)+'.jpg'
